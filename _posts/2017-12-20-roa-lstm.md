...
First Attempt: This approach uses an LSTM to generate character-by-character predictions after training on the Rules as a solid block of text.  
...
So, the character-by-character approach clearly is not working for us---the model has only learned to produce a verbatim replication of the input and we don;t get any of the neat novelty we were hoping for. Note: this is before any parameter tuning, choosing some sensible defaults (i.e., the `adam` optimizer), and setting the input to character-by-character. 

The biggest concern with LSTM on this project was that the data were simply too small to be represented with such a complex model. Presenting 100 character blocks as input might be increasing this identical text replication pattern. Stay tuned for the next iteration where a word-by-word LSTM will be examined.
